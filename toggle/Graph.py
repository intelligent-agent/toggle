...
def color(string): return Clutter.color_from_string(string)[1]    # shortcut
...
class GraphPlot():
    ...
    def draw(self, ctx, width, height):
        ...
        if len(self.times) == 1:
            ...
            y_values = height - (np.array(self.values) -
                                 self.scale_min) * (height / self.scale_tot)
...
class GraphScale():
    ...
    def draw(self, ctx, width, height):
        ...
        for y in self.y_values:
            ctx.move_to(23, height - (y - self.scale_min)
                        * (height / self.scale_tot))
            ctx.line_to(width, height - (y - self.scale_min)
                        * (height / self.scale_tot))
            ctx.move_to(0, height - (y - self.scale_min)
                        * (height / self.scale_tot))
            ctx.show_text(str(y))
        if self.title:
