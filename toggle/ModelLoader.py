...
class ModelLoader(Clutter.Actor):
    ...
    def load_models(self):
        ...
        if self.models.count() > 0:
            logging.debug("Found " + str(self.models.count()) +
                          " models in " + self.path)
    ...
    def sync_models(self):
        ...
        if not self.remotes:
            return
        remote_names = [f["name"]
                        for f in self.remotes["files"] if f["type"] == "model"]
    ...
    def download_model(self, f):
        ...
        if r.status_code == 200:
            ...
            try:
                ...
                logging.warning(
                    "ModelLoader: Unable to download file. Check permissions")

        else:
            logging.warning(
                "Unable to download file. Got response: " +
                r.status_code)

    def delete_model(self, name):
        model_path = join(self.path, name)
        logging.warning(
            "Syncing models with OctoPrint, deleting " +
            model_path)
        try:
            remove(model_path)
        except OSError as e:
            logging.error(
                "ModelLoader: Unable to delete file. Check permissions")
