...
class WebSocksClient():
    ...
    def __init__(self, config=None, host="ws://kamikaze.local"):
        ...
        self.url = host + \
            '/'.join([self._prefix, self._r1, self._conn_id, 'websocket'])
    ...
    def _on_message(self, msg):
        ...
        if data == 'o':
            ...
            logging.debug(
                "SockJS: Socket disconnected, reason: " +
                str(payload))
        elif data == 'h':
            ...
            logging.debug("Got garbled char " + str(msg) +
                          " hex: " + str(int(msg, 16)))
    ...
    def run(self):
        for i in range(self.config.getint("Rest", "timeout")):
            if self.running:
                self.config.splash.set_status(
                    "Connecting to {} ({})".format(self.host, i))
