...
do_plate_paint (ClutterActor *self){
    ...
    cogl_framebuffer_translate(priv->fb,  priv->width/2, priv->height/2, 0);
    cogl_framebuffer_scale (priv->fb, 100, 100, 100);
    cogl_primitive_draw (priv->prim, priv->fb, priv->pipeline);
    cogl_framebuffer_pop_matrix (priv->fb);    

    
    // Draw the frambuffer as the actor. TODO: change this with framebuffer cmds. 
