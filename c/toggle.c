...
void on_timeline_new_frame(ClutterTimeline *timeline, gint frame_num, gpointer data) {
        ...
        clutter_actor_set_rotation_angle(model, CLUTTER_Y_AXIS, rotation );
}
...
int main(int argc, char *argv[]) {
	...
	if ((model = mash_model_new_from_file (MASH_DATA_NONE, "../models/teapot.ply", &err))
	== NULL){
	...
	clutter_actor_add_child(plate, light_directional);
	clutter_actor_add_child(plate, light_spot);
